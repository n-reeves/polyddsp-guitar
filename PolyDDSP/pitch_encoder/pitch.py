...
import numpy as np
import PolyDDSP.pitch_encoder.nnAudio.features.cqt as nnAudio
import math
import PolyDDSP.pitch_encoder.utils as utils
...
def midi_pitch_to_contour_bins(pitch_midi: int) -> int:
    ...
    pitch_bin = 12.0 * contour_bins_per_semitone * torch.log2(pitch_hz / annotation_base)
    return int(torch.round(pitch_bin).item())


def get_pitch_bends(
    contours: torch.Tensor, note_event: list[int], n_bins_tolerance: int = 25, device: str = "cpu"
) -> torch.Tensor:
    """
    ...
    freq_gaussian = freq_gaussian.to(device)
...
def output_to_notes_polyphonic(
    ...
    for batch_idx, freq_idx, note_start_idx in onset_idx:
        ...
        bends = get_pitch_bends(
            contours, [batch_idx, freq_idx + MIDI_OFFSET, note_start_idx, i], 25, device
        )
    ...
    if melodia_trick:
        ...
        while torch.max(remaining_energy) > frame_thresh:
            ...
            bends = get_pitch_bends(
                contours, [batch, freq_idx + MIDI_OFFSET, i_start, i_end], 25, device
            )
...
class PitchEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.sr = sr
        self.device = device
        self.overlap_len = 30 * 256 #amount the current window overlaps with
        self.hop_size = 512
        self.n_overlapping_frames = 30
        self.resamp = torchaudio.transforms.Resample(orig_freq=self.in_sr, new_freq=self.sr)
        ...
        self.model.load_state_dict(torch.load("PolyDDSP/pitch_encoder/assets/bp_pytorch.pth"))
        self.model.eval()
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        #add overlap length zero padding to front of audio
